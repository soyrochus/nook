...
- [ ] 8.5 Verify end-to-end with a test tag (e.g. `v0.0.0-test`) before cutting a real release tag, then remove the test release/tag — **not done**: requires pushing a tag to a real GitHub remote and watching Actions run, which isn't possible from this environment. Do this before the first real release.
