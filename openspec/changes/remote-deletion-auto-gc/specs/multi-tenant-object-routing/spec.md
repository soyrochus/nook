...
**Reason**: SPEC-005 (remote deletion and automatic garbage collection) deliberately extends the object API with a `DELETE` verb and a namespace-scoped listing endpoint; the frozen three-verb surface is superseded.
**Migration**: The API surface is now exactly: `GET`/`HEAD`/`PUT`/`DELETE` on the per-object path plus `GET` on the namespace listing path. Vault- and namespace-management actions remain unreachable over HTTP.
