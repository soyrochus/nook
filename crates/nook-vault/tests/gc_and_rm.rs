...
fn put_orphan(
    ...
    let sig = nook_vault::sign_request(&credential, "PUT", &path, timestamp, body);
...
fn push_reclaims_replaced_content_and_quota_matches_disk() {
    ...
    assert_eq!(
        files.len(),
        2,
        "old content object must be swept: {files:?}"
    );
    ...
    assert_eq!(
        fs::read(vault_root.join("a.txt")).unwrap(),
        b"version two, somewhat longer"
    );
}
...
fn rm_removes_files_and_subtrees_remotely_only() {
    ...
    assert!(
        vault_root.join("docs/b.txt").exists(),
        "rm must never touch local files"
    );
    ...
    assert!(
        !tree_out.contains("docs"),
        "tree still shows docs: {tree_out}"
    );
...
fn grace_window_protects_fresh_orphans_and_reclaims_old_ones() {
    ...
    assert!(
        orphan_path.exists(),
        "fresh orphan must survive the grace window"
    );
...
fn manifest_head_survives_emptying_the_namespace() {
    ...
    assert!(
        ls.status.success(),
        "ls after emptying must still work: {ls:?}"
    );
}
