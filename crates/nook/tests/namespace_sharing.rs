...
fn exported_namespace_can_be_imported_by_a_second_client() {
    ...
    assert!(
        run_nook(&fred_config, &["root", "--set", fred_root.to_str().unwrap()]).status.success()
    );
    assert!(run_nook(&fred_config, &["push"]).status.success(), "fred push failed");

    let export = run_nook(&fred_config, &["namespace", "export"]);
    assert!(export.status.success(), "namespace export failed: {export:?}");
    let bundle = String::from_utf8_lossy(&export.stdout).trim().to_string();
    assert!(bundle.starts_with("nookns1:"), "unexpected bundle format: {bundle}");
    ...
    assert!(mary_init.status.success(), "mary init failed: {mary_init:?}");
    assert!(
        run_nook(&mary_config, &["root", "--set", mary_root.to_str().unwrap()]).status.success()
    );

    let pull = run_nook(&mary_config, &["pull"]);
    assert!(pull.status.success(), "mary pull failed: {pull:?}");

    let pulled = fs::read_to_string(mary_root.join("shared.txt")).expect("shared.txt pulled by mary");
...
fn independent_namespaces_cannot_read_each_others_data() {
    ...
    assert!(!mary_pull.status.success(), "mary must not see fred's namespace: {mary_pull:?}");
